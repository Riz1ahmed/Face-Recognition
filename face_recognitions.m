...
random_Index=round(30*rand(1,1));
...
if (random_Index <= 10)
    name='Emdad';
elseif (random_Index <= 20)
    name='Pabel';
else
    name='Rizwan';
end

title_text = strcat({'Search for '}, name);

subplot(121);
imshow(reshape(random_Image,112,92));
title(title_text,'FontWeight','bold','Fontsize',16,'color','red');
subplot(122);
...
subplot(122);
imshow(reshape(rest_of_the_images(:,i),112,92));
title('Resulted Face','FontWeight','bold','Fontsize',16,'color','red');